...
export async function PUT(request: NextRequest) {
  ...
  const tags = Array.isArray(body.tags) ? body.tags.filter((tag: unknown): tag is string => typeof tag === "string").map((tag) => tag.trim()).filter(Boolean).slice(0, 8) : fallback.tags;
